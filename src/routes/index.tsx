...
function BlogPreviewSection() {
  ...
  const posts = [
    { category: "Branding", title: "Como criar uma identidade visual memorável", excerpt: "Os princípios fundamentais para construir uma marca que perdure." },
    { category: "Web Design", title: "Tendências de web design para 2026", excerpt: "O que esperar do design digital no próximo ano." },
    { category: "UX", title: "A importância da experiência do utilizador", excerpt: "Como o UX design impacta diretamente as conversões." },
  ];
  ...
  return (
    <section className="py-24 sm:py-32" ref={ref}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeader
          eyebrow="Blog"
          title="Insights e inspiração"
          description="Artigos sobre design, branding e estratégia digital."
        />
        <div className={`reveal ${isVisible ? "visible" : ""} mt-16 grid gap-6 lg:grid-cols-3`}>
          {posts.map((post, index) => (
            <Card
              key={post.title}
              className="group glass cursor-pointer border-border/50 bg-card/40 transition-all hover:-translate-y-1 hover:border-turquoise/30"
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <CardContent className="p-6">
                <span className="text-xs font-semibold uppercase tracking-wider text-turquoise">
